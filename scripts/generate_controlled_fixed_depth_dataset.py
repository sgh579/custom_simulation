...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--global-index-min", type=int, default=None, help="Only generate samples with global_index >= this value.")
    parser.add_argument("--global-index-max", type=int, default=None, help="Only generate samples with global_index <= this value.")
    parser.add_argument("--status-tag", type=str, default=None, help="Optional suffix for per-worker status/event files.")
...
def main() -> None:
    ...
    if args.global_index_min is not None:
        selected = [row for row in selected if row.global_index >= int(args.global_index_min)]
    if args.global_index_max is not None:
        selected = [row for row in selected if row.global_index <= int(args.global_index_max)]
    if args.max_samples is not None:
        selected = selected[: max(int(args.max_samples), 0)]
    status_tag = f"_{args.status_tag}" if args.status_tag else ""
    status_path = args.out_dir / "status" / f"worker_{args.worker_index:03d}{status_tag}_status.json"
    events_path = args.out_dir / "logs" / f"worker_{args.worker_index:03d}{status_tag}_events.jsonl"
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.global_index_min is not None and int(args.global_index_min) < 0:
        raise SystemExit("--global-index-min must be non-negative")
    if args.global_index_max is not None and int(args.global_index_max) < 0:
        raise SystemExit("--global-index-max must be non-negative")
    if (
        args.global_index_min is not None
        and args.global_index_max is not None
        and int(args.global_index_max) < int(args.global_index_min)
    ):
        raise SystemExit("--global-index-max must be >= --global-index-min")
